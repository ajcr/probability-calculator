...
def test_probability_permutations_no_adjacent(runner, sequence, expected, expected_if_same_distinct):
    result = runner.invoke(permutation, [sequence, "--constraints", "no_adjacent"])
    assert result.output.rstrip() == str(expected)
    result = runner.invoke(permutation, [sequence, "--constraints", "no_adjacent", "--same-distinct"])
...
def test_count_permutations_derangement(runner, sequence, expected, expected_if_same_distinct):
    ...
    assert result.output.rstrip() == str(expected)
    result = runner.invoke(permutation, [sequence, "--constraints", "derangement", "--same-distinct"])
    assert result.output.rstrip() == str(expected_if_same_distinct)
